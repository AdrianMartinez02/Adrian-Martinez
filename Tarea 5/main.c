...
const gpio_num_t GPIOs[] = {
    LSAP, LSCP, OPEN, CLOSE, STOP, MO_OPEN, MO_CLOSE, LAMPARA, BUZZER, PPP
};
...
void TimerCallback (void * arg)
{
    ...
    for(int i = 0; i < sizeof(GPIOs)/sizeof(gpio_num_t); i++) {
        sprintf(payload, "El GPIO %d está en %d", GPIOs[i], gpio_get_level(GPIOs[i]));
...
void LAMPARA_ON_OFF(void * arg)
{
    ...
    {
            gpio_set_level(LAMPARA, !gpio_get_level(LAMPARA)); 
...
void TimerConfig()
{
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(periodic_timer_1, 50000)); // 100 ms
